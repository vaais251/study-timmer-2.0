...
const Controls: React.FC<ControlsProps> = ({ isRunning, startTimer, stopTimer, resetTimer, timeRemaining, sessionTotalTime, mode }) => {
        ...
        return (
            <div className="text-center my-6 h-16 flex items-center justify-center">
                <button
                    ...
                    className={`text-white font-semibold py-4 px-12 rounded-full shadow-lg transition-all transform hover:scale-105 focus:outline-none focus:ring-4 ${buttonClass}`}
    ...
    const ActionButton: React.FC<{ onClick: () => void; label: string; children: React.ReactNode; className: string; }> = ({ onClick, label, children, className }) => (
        <button
            onClick={onClick}
            className={`w-36 h-16 rounded-full text-white font-semibold flex items-center justify-center gap-2 transition-all duration-200 hover:-translate-y-1 focus:outline-none focus:ring-4 focus:ring-white/30 shadow-lg ${className}`}
            aria-label={label}
        >
            {children}
            <span>{label}</span>
    ...
    const SecondaryButton: React.FC<{ onClick: () => void; label: string; children: React.ReactNode; className: string; }> = ({ onClick, label, children, className }) => (
        <button
            onClick={onClick}
            className={`w-16 h-16 rounded-full text-slate-300 hover:text-white flex items-center justify-center transition-all duration-200 hover:bg-white/10 focus:outline-none focus:ring-4 focus:ring-white/20 shadow-md ${className}`}
            aria-label={label}
        >
            {children}
        </button>
    ...
    const MainActionButton = isRunning 
        ? (
            <ActionButton onClick={handleMainClick} label="Pause" className="bg-amber-500 hover:bg-amber-600">
                <PauseIcon />
            </ActionButton>
        ) : ( // isPaused
             <ActionButton onClick={handleMainClick} label="Resume" className="bg-green-500 hover:bg-green-600">
                <PlayIcon />
            </ActionButton>
        );
